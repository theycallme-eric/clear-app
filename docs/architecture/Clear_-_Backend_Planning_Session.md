...
**Note on POWER Anchor:** Includes explosive/Olympic movements like Power Clean, Squat Clean, Hang Clean, Thruster. These movements can also appear in FULL BODY workouts when anchor is not selected during onboarding.
...
interface MovementPattern {
  ...
  anchor: AnchorType | null;     // Which anchor this maps to (null for Core)
  description: string;
}
```
...
interface ExerciseDefinition {
  ...
  can_be_primary: boolean;       // Can this serve as primary lift?
}
```
...
└── MOVEMENT PATTERNS
    ...
    │   ├── squat-clean [barbell]
    │   ├── hang-clean [barbell]
    │   └── thruster [barbell, dumbbell]
    │
    └── power-accessory
        ├── db-snatch [dumbbell]
        ├── kb-snatch [kettlebell]
        └── sotts-press [dumbbell, kettlebell]

SECTION: CORE (No specific anchor)
└── MOVEMENT PATTERNS
    ├── anti-rotation
    ...
    │
    ├── rotation
    │   ├── russian-twist [kettlebell, dumbbell, medicine_ball]
    │   ├── woodchop [cable, dumbbell]
    │   └── landmine-rotation [barbell]
    │
    └── flexion-extension
        ├── sit-up [bodyweight, medicine_ball]
