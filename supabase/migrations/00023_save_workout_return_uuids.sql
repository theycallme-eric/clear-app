-- Migration: Update save_generated_workout to return section/exercise UUIDs
-- Previously returned only the session UUID. Now returns JSONB with the full
-- UUID mapping so the frontend can use real DB IDs for exercise logging.
