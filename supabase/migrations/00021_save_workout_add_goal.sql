-- Migration: Update save_generated_workout RPC to accept goal_preset parameter
-- This allows the generation flow to persist the goal used for each workout

-- Drop old function signature first (different param count = different overload in PG)
DROP FUNCTION IF EXISTS save_generated_workout(UUID, UUID, DATE, anchor_type, SMALLINT, JSONB, SMALLINT, TEXT);
